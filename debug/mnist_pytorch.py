...
class CNN(object):
    ...
    def fit(self, train_idx, train_labels=None, sample_weight=None, loader='train'):
        ...
        ** All inputs should be numpy arrays, not pyTorch Tensors
        ...
        if sample_weight is not None:
            ...
            class_weight = sample_weight[torch.unique(torch.from_numpy(train_labels), return_inverse =True)[1]]
        ...
        if train_labels is not None:
            ...
            sparse_labels = torch.zeros(MNIST_TRAIN_SIZE if loader == 'train' else MNIST_TEST_SIZE, dtype=torch.int64) - 1
            sparse_labels[train_idx] = torch.from_numpy(train_labels)
    ...
    def test(self):
        ...
        target = datasets.MNIST('/home/zhaok14/example/PycharmProjects/cnn_scratch/MNIST_data', train=False, transform=transforms.Compose([
            transforms.ToTensor(), transforms.Normalize((0.1307,), (0.3081,))
        ])).test_labels.numpy()

        pred = self.predict(loader='test')
        correct = np.count_nonzero(pred == target)
    ...
    def test_deprecated(self):
        ...
        test_loader = torch.utils.data.DataLoader(
            datasets.MNIST('/home/zhaok14/example/PycharmProjects/cnn_scratch/MNIST_data', train=False, transform=transforms.Compose([
                transforms.ToTensor(),
...
if __name__ == '__main__':
    y_train = datasets.MNIST('/home/zhaok14/example/PycharmProjects/cnn_scratch/MNIST_data', download=True, train=True).train_labels.numpy()
    y_test = datasets.MNIST('/home/zhaok14/example/PycharmProjects/cnn_scratch/MNIST_data', train=False).test_labels.numpy()

    cnn = CNN(epochs=10)

    mod_val = 1
    train_idx = torch.arange(MNIST_TRAIN_SIZE)[torch.arange(MNIST_TRAIN_SIZE) % mod_val == 0]
    train_labels = y_train[torch.arange(MNIST_TRAIN_SIZE) % mod_val == 0]
    # sample_weight = torch.ones(len(train_labels)) / len(train_labels)
    # print(torch.ones(len(train_labels)))
    # print(sample_weight)
    # print(sample_weight.size())
    #
    # temp = train_labels
    # temp = temp.astype(np.float64)
    # temp[:] = 1.0/len(train_labels)
    # temp[(train_labels == 9) | (train_labels == 8)] *= 5
    # sample_weight = torch.FloatTensor(temp)
    #
    # print(sum(sample_weight)) #errorneous generation
    #
    # # sample_weight[(train_labels == 9) | (train_labels == 8)] *= 5
    # sample_weight = sample_weight / sum(sample_weight)

    # cnn.fit(train_idx, train_labels, sample_weight=None )
    cnn.fit(train_idx, train_labels = None , sample_weight=None)
    # cnn.model = m
    # m = cnn.model = m
    ...
    # torch.bincount(y_test[pred == y_test]) / torch.bincount(y_test).astype(float)

    gc.collect()

    rp = RankPruning(clf=CNN(epochs=5))
    rp.fit(train_idx, train_labels)
    rp.predict(loader='test')
