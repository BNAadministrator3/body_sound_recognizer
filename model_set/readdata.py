...
MAX_AUDIO_LENGTH=650 #train:2152, eval:1774
...
class DataSpeech():
	...
	def data_genetator(self, batch_size=32, audio_length = MAX_AUDIO_LENGTH):
		...
		while True:
			...
			if batch_size == 1:
				data_input, data_labels = self.GetData(n_start=ran_num )  # 从随机数开始连续向后取一定数量数据
				data_input = data_input.reshape(1,data_input.shape[0],data_input.shape[1],data_input.shape[2])
				data_labels = data_labels.reshape(1,)
				yield [data_input, keras.utils.to_categorical(data_labels, num_classes = self.class_num)],keras.utils.to_categorical(data_labels, num_classes = self.class_num)
			else:
