...
class PlotLosses(kr.callbacks.Callback):
...
class ModelSpeech():  # 语音模型类
    ...
    def CreateModel(self):
        ...
        '''
        self.input_data = Input(name='the_input', shape=(self.AUDIO_LENGTH, self.AUDIO_FEATURE_LENGTH, 1))
        ...
        self.layer_r12 = Flatten(name='flatten')(self.layer_h12)

        layer_d13 = Dropout(0.4)(self.layer_r12)
        ...
        self.y_pred = Dense(self.MS_OUTPUT_SIZE, use_bias=True, kernel_initializer='he_normal', activation='softmax')(
            layer_d14)  # 全连接层
        ...
        model.compile(loss={'softmax_loss': lambda y_true, y_pred: y_pred}, optimizer=ada_d,)
                      # metrics=[false_positives, false_negatives, true_positives, true_negatives])
    ...
    def softmax_lambda_func(self, args):
        ...
        # loss = -labels * (1 - tf.clip_by_value(y_pred, 1e-10, 1.0)) * (
        #             1 - tf.clip_by_value(y_pred, 1e-10, 1.0)) * K.log(tf.clip_by_value(y_pred, 1e-10, 1.0))
    ...
    def TrainModel(self, datapath, epoch=2, batch_size=32, load_model=False, filename='model_set/speech_model25'):
        ...
        for epoches in range(epoch):  # 迭代轮数
            ...
            self.SaveModel(comment='_new_' + str(epoches + prepoch + 1) + '_steps_' + str(
                (epoches + prepoch + 1) * iterations_per_epoch))
        ...
        try:
            ...
            for i in range(data_count):
                ...
                while (data_input.shape[0] > self.AUDIO_LENGTH):
                    print('*[Error]', '.wav data with NO. ', (ran_num + i) % num_data, 'is too long with length ',
                          data_input.shape[0], ',\n A Exception raises when test Speech Model.')
                    num_bias += 1
                    data_input, data_labels = data.GetData((ran_num + i + num_bias) % num_data,
                                                           mode='non-repetitive')  # 从随机数开始连续向后取一定数量数据

                data_in = np.zeros((self.AUDIO_LENGTH,self.AUDIO_FEATURE_LENGTH,1), dtype=np.float)
                data_in[0:len(data_input)] = data_input

                if (i % 100 == 0 and show_ratio == True):
                    print('测试进度：', i, '/', data_count)

                data_pre = self.Predict(data_in)
