...
class ModelSpeech():  # 语音模型类
    def __init__(self, datapath):
        ...
        self.metrics = {'order':0,'type':'eval','sensitivity':0,'specificity':0,'score':0,'accuracy':0, 'epoch':0}

    def CreateModel(self, graph = tf.get_default_graph(), order = 0):
        ...
        with self.graph.as_default():
            ...
            self.fc_2 = tf.layers.dense(droput_10, self.MS_OUTPUT_SIZE, activation=None, use_bias=True,#no activation
                                   kernel_initializer=tf.keras.initializers.he_normal(),
                                   name='fc_2')
            ...
            with tf.name_scope('fold_' + str(order) ):
    ...
    def TrainModel(self, datapath, epoch=2, batch_size=32, load_model=False, filename='model_set/speech_model25'):
        ...
        for fold in range(FOLDER_SPLIT_NUM):
            ...
            print(90 * '*')
            print(90 * '*')
            print('\nthe {}th cross validation'.format(fold))
            gc.collect()
            ...
            #1. create graph
            self.graph = tf.Graph()
            #2. create computation nodes
            model_summary = self.CreateModel(graph=self.graph,order=fold)
            # 3. create summary monitor
            # as above
            #4. create session to run graph
            with tf.Session(graph=self.graph) as sess:
                ...
                if load_model == True:
                    try:
                        saver.restore(sess, os.path.join(os.getcwd(), 'checkpoints', 'files_model', 'speech-f'+str(fold)+'.module')) #two files in a folder.
                    except:
                ...
                for i in range(0, epoch):
                    ...
                    for input, _ in pbar:
                        ...
                        _, loss, train_summary = sess.run([self.optimize, self.loss, model_summary], feed_dict=feed)
                    ...
                    pbar.close()
                    if i%2 == 0:
                        self.TestModel(sess=sess, dataObjec=data, ronds=fold, type='train', data_count=-1, out_report=False, writer=train_writter, step=i)
                        metrics = self.TestModel(sess=sess, dataObjec=data, ronds=fold, type='eval', data_count=-1, out_report=False, writer=train_writter, step=i)
                        if (metrics['score'] > best_score and i>19):
                            best_score = metrics['score']
                            self.metrics = metrics
                            self.metrics['epoch'] = i
                            saver.save(sess, os.path.join(os.getcwd(),'checkpoints','files_model', 'speech-f'+str(fold)+'.module'), global_step=i)
            ...
            metrics_info.append(self.metrics)

        sens = 0
        speci = 0
        score = 0
        accur = 0
        for mt in metrics_info:
            sens = sens + mt['sensitivity']
            speci = speci + mt['specificity']
            score = score + mt['score']
            accur = accur + mt['accuracy']
        sens = sens / FOLDER_SPLIT_NUM
        speci = speci / FOLDER_SPLIT_NUM
        score = score / FOLDER_SPLIT_NUM
        accur = accur / FOLDER_SPLIT_NUM
        print('\n5-fold cross validation results: ')
        print('spec:',print(metrics_info))
        print('Average metric:: Sensitivity: {}; Specificity: {}; Score: {}; Accuracy: {}'.format(sens,
                                                                                 speci,
                                                                                 score,
                                                                                 accur))

    def TestModel(self, sess, dataObjec, ronds, type, data_count=32, out_report=False, show_ratio=True,writer=tf.summary.FileWriter('files_summary', tf.get_default_graph()),step=0):
        '''
        测试检验模型效果
        '''
        dataObjec.SplitType(type=type, order=ronds)

        if type == 'train':
            num_data = sum(dataObjec.DataNum)  # 获取数据的数量
        elif type == 'eval':
            num_data = dataObjec.DataNum
        else:
            print('[ERROR] uncompatible type.')
            assert(0)
        ...
        try:
            ...
            txt_obj = []
            if (out_report == True):
                txt_obj = open('Test_Report_fold_' + str(ronds) +  '_' + type + '_' + nowtime + '.txt', 'w', encoding='UTF-8')  # 打开文件并读入
            ...
            for i in tqdm(range(data_count)):
                data_input, data_labels = dataObjec.GetData((ran_num + i) % num_data,
            ...
            strg = '*[测试结果] 片段识别 fold_{6}_{0} 敏感度：{1}%, 特异度： {2}%, 得分： {3}, 准确度： {4}%, 用时: {5}s.'.format(type, sensitivity, specificity, score, accuracy, dtime, ronds)
            tqdm.write(strg)

            assert(len(cm_lab) == len(cm_pre))
            img_cm = self.plot_confusion_matrix(cm_lab, cm_pre, list(map.values()),
                                  tensor_name='fold_'+ str(ronds) + '/' + type +'/confusion-matrix', normalize=False)
            writer.add_summary(img_cm,global_step=step)
            summary = tf.Summary()
            summary.value.add(tag= 'fold_'+ str(ronds) + '_' + type + '/sensitivity',simple_value=sensitivity )
            summary.value.add(tag= 'fold_'+ str(ronds) + '_' + type + '/specificity',simple_value=specificity )
            summary.value.add(tag= 'fold_'+ str(ronds) + '_' + type + '/score', simple_value=score )
            summary.value.add(tag= 'fold_'+ str(ronds) + '_' + type + '/accuracy', simple_value=accuracy)
            writer.add_summary(summary,global_step=step)

            if (out_report == True):
                txt = '*[测试结果] 片段识别 fold_' + str(ronds) +'_'+ type + ' 敏感度：' + sensitivity + '%, 特异度： ' + specificity + '%, 得分： ' + score + ', 准确度： ' + accuracy + '%, 用时: ' + dtime + 's.'
                txt_obj.write(txt)
                txt_obj.close()

            metrics = {'order':ronds,'type':type,'sensitivity':sensitivity,'specificity':specificity,'score':score,'accuracy':accuracy}
