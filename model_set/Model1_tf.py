...
from model_set.readdata import MAX_AUDIO_LENGTH, AUDIO_FEATURE_LENGTH, CLASS_NUM
...
class ModelSpeech():  # 语音模型类
    ...
    def TrainModel(self, datapath, epoch=2, batch_size=32, load_model=False, filename='model_set/speech_model25'):
        ...
        with tf.Session() as sess:
            ...
            try:
                saver.restore(sess, os.path.join(os.getcwd(), 'files_model', 'speech.module')) #two files in a folder.
            except:
                print('Loading weights failed. Train from scratch.')
            summary_merge = tf.summary.merge(self.train_summary)
            os.system('rm -rf files_summary ')
            train_writter = tf.summary.FileWriter('files_summary', sess.graph)
            for i in range(0, epoch):
                ...
                self.TestModel(sess=sess, datapath=self.datapath, str_dataset='eval', data_count=-1, out_report=False, writer=train_writter, step=i)
                saver.save(sess, os.path.join(os.getcwd(),'files_model', 'speech.module_n'), global_step=i)
    ...
    def TestModel(self, sess, datapath='', str_dataset='eval', data_count=32, out_report=False, show_ratio=True,writer=tf.summary.FileWriter('files_summary', tf.get_default_graph()),step=0):
        ...
        try:
            ...
            for i in range(data_count):
                ...
                data_in=data_in.reshape((1,data_in.shape[0],data_in.shape[1],data_in.shape[2]))
                ...
                tp, fp, tn, fn = Comapare2(np.argmax(data_pre[0][0],axis=0), data_labels[0])  # 计算metrics
                overall_p += tp + fn
                overall_n += tn + fp
                overall_tp += tp
                overall_tn += tn
